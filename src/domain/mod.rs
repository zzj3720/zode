...
impl SessionState {
        ...
        I: IntoIterator<Item = EventRecord>,
    {
        records
            .into_iter()
            .try_fold(Self::new(session_id), |state, record| state.apply_record(&record))
    }
                    ...
                    for call in &message.tool_calls {
                        if self.transcript.iter().any(|existing| {
                            existing
                                .tool_calls
                                .iter()
                                .any(|existing_call| existing_call.tool_call_id == call.tool_call_id)
                        }) {
                            return Err(DomainError::DuplicateTranscriptToolCallId(
                                call.tool_call_id.clone(),
                            ));
...
fn validate_started_record(record: &AsyncToolCallRecord) -> Result<(), DomainError> {
    if !matches!(record.status, AsyncToolStatus::Planned | AsyncToolStatus::Running)
...
fn validate_async_record(record: &AsyncToolCallRecord) -> Result<(), DomainError> {
    ...
    match record.status {
        ...
        AsyncToolStatus::RuntimeRestarted => {
            ...
            {
                return Err(DomainError::InvalidState(
                    "runtime-restarted async tool calls need only error and completion time"
                        .into(),
                ));
            }
