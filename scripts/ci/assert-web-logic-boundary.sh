...
if rg -n "(from|import\\()[[:space:]]*['\"](?:react|react-dom|@emotion|@radix-ui|@preact/signals-react)" \
  "$ROOT/web/src/logic"; then
  die 'domain logic imports a visual framework'
fi

if rg -n '\b(?:fetch|EventSource|WebSocket)\b' "$ROOT/web/src/logic"; then
  die 'domain logic bypasses the single Server client'
fi

if rg -n '(AbortController|Last-Event-ID|endpointEvents|CursorStore|\bcursor\b)' \
  "$ROOT/web/src/logic/session.ts"; then
  die 'Session owns stream, cursor, or reconnect state'
fi

server_client_roots=$(rg -l 'new ServerClient\(' "$ROOT/web/src" || true)
...
if find "$ROOT/web/src" -maxdepth 1 -type f \( -name '*.ts' -o -name '*.tsx' \) -print0 |
  xargs -0 rg -n "(\\bfetch\\(|\\bEventSource\\b|\\bWebSocket\\b|ServerClient|from[[:space:]]+['\"]\\./api)"; then
  die 'visual entry owns transport or imports the Server API client'
fi

if rg -n '/v1/(?:endpoints/[^/]+/)?sessions/[^/]+/events' \
