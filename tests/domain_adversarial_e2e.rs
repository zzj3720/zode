...
async fn create_model_less(
    ...
    if status != StatusCode::CREATED {
        return Err(
            Error::other(format!("model-less session create failed: {status} {body}")).into(),
        );
    }
    require_ulid(&body)
}
...
async fn e2e_session_list_keyset_is_owner_bound_and_restart_stable() -> TestResult<()> {
    ...
    let (status, body, second_before_restart) = list_page(
        &client,
        &server,
        SUBJECT,
    ...
    assert_eq!(status, StatusCode::OK, "{body}");
    assert_eq!(
        second_before_restart["schema"], "zode.session-list.v1",
        "{body}"
    );
    ...
    assert!(created
        .iter()
        .all(|id| { first_ids.iter().any(|first_id| first_id == id) || second_id == id }));

    let (cross_status, cross_body, cross_json) = list_page(
        &client,
        &server,
        OTHER_SUBJECT,
        &format!("limit=2&cursor={cursor}"),
    ...
    assert_eq!(
        cross_json["error"]["code"], "malformed_request",
        "{cross_body}"
    );
    ...
    let (status, body, second_after_restart) = list_page(
        &client,
        &restarted,
        SUBJECT,
        &format!("limit=2&cursor={cursor}"),
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn e2e_session_list_accepts_documented_limit_200_for_owner_cursor() -> TestResult<()> {
    let database = TempDatabase::new("domain-list-limit-200")?;
    let config = write_endpoint_config(&database, Vec::new(), 1)?;
    let mut server = ConfiguredServer::start(&database, &config).await?;
    let client = support::http_client()?;

    let first = create_model_less(&client, &server, SUBJECT, "list-limit-200-a").await?;
    let second = create_model_less(&client, &server, SUBJECT, "list-limit-200-b").await?;
    let (status, body, page) = list_page(&client, &server, SUBJECT, "limit=200").await?;
    assert_eq!(status, StatusCode::OK, "{body}");
    let items = page["items"]
        .as_array()
        .ok_or_else(|| Error::other(format!("limit=200 response omitted items: {body}")))?;
    assert_eq!(items.len(), 2, "{body}");
    assert!(items.iter().any(|item| item["session_id"] == first));
    assert!(items.iter().any(|item| item["session_id"] == second));
    assert!(page["next_cursor"].is_null(), "{body}");
...
async fn e2e_invalid_transition_rejects_without_append() -> TestResult<()> {
    ...
    let client = support::http_client()?;
    let session_id =
        create_model_less(&client, &server, SUBJECT, "invalid-transition-create").await?;
