...
const E2E_NAME = "e2e_browser_endpoint_unavailable_does_not_render_empty_sessions";
const CLASSIFICATION = "ENDPOINT_UNAVAILABLE_RENDERED_AS_EMPTY_SESSIONS";
const FIRST_OBSERVED =
  "the real Endpoint stopped responding, but the Sessions page rendered No sessions yet instead of an explicit Endpoint unavailable non-authoritative state";
const PROVIDER = "session-unavailable-fixture";
const MODEL = "session-unavailable-model";
const ENDPOINT_LABEL = "Unavailable sessions Endpoint";
const INCIDENT_DIRECTORY = path.resolve(__dirname, "../fixtures/incidents");
const CAPTURE_ENV = "ZODE_CAPTURE_ENDPOINT_UNAVAILABLE_SESSIONS";
...
async function preseedSession(page, harness) {
  ...
  const profile = await managementJson(page, "POST", `/v1/providers/${PROVIDER}/auth-profiles`, {
    kind: "api_key",
    label: "Unavailable sessions profile",
...
test(E2E_NAME, async ({ page }) => {
  ...
  const harness = await createWebE2EHarness({
    ...
    authorityId: "web-e2e-endpoint-unavailable",
  });
  let captureSetId;
  let primaryError;
  try {
    ...
    try {
      ...
      await expect(page.getByText(/Endpoint (?:is )?unavailable/iu)).toBeVisible();
      await expect(page.getByText("No sessions yet", { exact: true })).toBeHidden();
    } catch (error) {
    ...
    try {
      ...
      if (!firstFailure) throw new Error("endpoint-unavailable capture contained no public exchange");
      if (primaryError) {
