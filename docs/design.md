...
| Provider execution | installed replica drives local aimux directly; descriptor schema is exact while its positive controller-assigned revision and bounded credential-free options round-trip unchanged across Server proxying and Endpoint restart; credential-bearing URLs have no side effects; rotation affects the next request while an in-flight request keeps its captured revision; shipped native and OpenAI-compatible adapters preserve their distinct wire/auth paths | `e2e_unknown_provider_execution_schema_is_rejected_and_revision_round_trips`; `e2e_credential_bearing_model_base_url_is_rejected_without_side_effects`; `e2e_server_forwards_and_endpoint_persists_provider_execution_options`; `e2e_native_anthropic_messages_stream_uses_exact_replica_and_sse`; deterministic fake-provider roundtrip/retry cases; opt-in `e2e_live_opencode_provider_roundtrip_and_restart` |
