...
| Provider execution | installed replica drives local aimux directly; descriptor schema is exact while its positive controller-assigned revision round-trips unchanged; credential-bearing URLs have no side effects; rotation affects the next request while an in-flight request keeps its captured revision | `e2e_unknown_provider_execution_schema_is_rejected_and_revision_round_trips`; `e2e_credential_bearing_model_base_url_is_rejected_without_side_effects`; deterministic fake-provider roundtrip/retry cases; opt-in `e2e_live_opencode_provider_roundtrip_and_restart`; rotation still requires its named red case before implementation |
