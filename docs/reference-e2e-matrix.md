...
| SSE reconnect and multiple clients | Server forwards Endpoint event IDs and `Last-Event-ID`; it stores no cursor, and reconnect does not duplicate a durable final. | `e2e_browser_session_admission_is_separate_from_completion_and_last_event_id_reconnect_replaces_provisional_final`; `e2e_browser_two_actor_session_isolation`; `e2e_browser_two_actor_session_isolation_replays_complete_endpoint_transcript`; `e2e_browser_provider_profiles_shared_deployment_replays_complete_endpoint_transcript`; `e2e_create_message_sse_reconnect_get_restart` | ANCHOR/PARTIAL: public reconnect and two-actor isolation are covered. The two explicit replay identities reload secret-safe immutable later-reproduction transcripts and consume every Server-to-Endpoint request/response/terminal outcome through real processes; the approved-product manifest makes missing or corrupt transcripts fail the aggregate gate. The original shallow incident cassettes remain unchanged as provenance. Combine with the long-task restart journey before declaring complete. |
...
pinned by
`scripts/ci/approved-product-playwright-manifest.json` to 25 files and 56 test
