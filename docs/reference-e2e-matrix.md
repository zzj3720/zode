...
| Endpoint-wide SSE reconnect and multiple sessions | One browser application owns one stream/cursor per Endpoint, receives multiple owned sessions on it, and dispatches by `session_id`; Server forwards Endpoint event IDs and `Last-Event-ID`, stores no cursor, and reconnect does not miss or duplicate either session's durable final. | `e2e_endpoint_event_stream_multiplexes_owned_sessions_and_reconnects_once`; `e2e_browser_endpoint_stream_multiplexes_sessions_across_navigation_and_reconnect`; `e2e_browser_two_actor_session_isolation`; `e2e_create_message_sse_reconnect_get_restart` | CANDIDATE GREEN: the real Endpoint and browser anchors use the Endpoint-wide routes and have passed in the current candidate; combined long-task restart, exact-main merge, and fixed-install acceptance remain required. |
| Context growth and resource bounds | Keep the complete append-only transcript available while bounding each provider context generation by tokens. Before the selected model's input budget is exhausted, the current agent writes a durable handoff document; Endpoint starts a fresh context for the same task without implicitly injecting old history or the document body; the successor agent reads the handoff and paginated original history through runtime-owned read-only tools. Input admitted during a handoff reaches the first fresh-context request, and restart retries the exact prepared handoff envelope. Model/tool rounds have no numeric task or activation ceiling. | `e2e_oversized_tool_output_uses_secret_safe_blob_reference`; `e2e_long_task_continues_until_final`; `e2e_long_task_writes_handoff_and_continues_in_fresh_context`; `e2e_delivery_admitted_during_handoff_reaches_first_fresh_context`; `e2e_handoff_restart_replays_frozen_request_and_queued_input`; `e2e_context_handoff_restart_reuses_committed_document`; `e2e_public_500_redaction` | CANDIDATE GREEN: one session crossed two context generations, actively read each handoff and original history, preserved complete public history, consumed input admitted during handoff in the first fresh context, replayed a frozen handoff request across Endpoint restart, avoided duplicate external effects, and reached one durable final. Exact-main and fixed-install acceptance remain pending. |
